...
from apps.api.app.db.models import IntelligenceRun,ResearchFindingRecord,TaskPlanRecord,SpecialistBriefRecord,PromptArtifactRecord,ModelInvocation
...
async def create_run(db:AsyncSession,request:str,project_id:UUID|None=None,task_id:UUID|None=None):
    row=IntelligenceRun(request=request,project_id=project_id,task_id=task_id,status="pending",current_stage="intent",result={}); db.add(row); await db.commit(); await db.refresh(row); return row
...
async def save_result(db:AsyncSession,row:IntelligenceRun,result:IntelligenceResult):
    row.status=result.status.value; row.current_stage=result.current_stage.value; row.result=result.model_dump(mode="json"); row.error=result.error; row.completed_at=datetime.now(timezone.utc) if result.status.value in {"completed","failed"} else None
